...
$SUDO_EXEC /bin/sh -c ". $target_sdk_dir/environment-setup-@MACHINE@ && \
	        make HOSTCC=\"\$HOSTCC\" -C $target_sdk_dir/sysroots/@REAL_MULTIMACH_TARGET_SYS@/usr/src/kernel/ prepare scripts tools >/dev/null"
