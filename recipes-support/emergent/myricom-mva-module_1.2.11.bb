...
RPROVIDES:${PN} += "kernel-module-myricom-mva"
...
SRC_URI:append = " \
	...
	file://90-myri-mva.network \
 	file://myri-mva.conf \
 	file://0002-fix-bad-udelay-undefined.patch \
 	"
...
do_install() {
	...
	install	-m 0644 ${WORKDIR}/90-myri-mva.network ${D}${sysconfdir}/systemd/network/

        install -d ${D}${sbindir}
        install -m 0755 ${S}/opt/EVT/myricom_mva/sbin/myri_create_devs ${D}${sbindir}
}
