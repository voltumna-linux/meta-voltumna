...
cd $TEMPDIR
rsync -azchHEADX --delete --only-write-batch=$ORIGDIR/$INCRUPDATE $TOBASEIMAGE-$TOVER/ $FROMBASEIMAGE-$FROMVER/
rsync -azchHEADX --delete --only-write-batch=$ORIGDIR/$FULLUPDATE $TOBASEIMAGE-$TOVER/ empty/
cd $ORIGDIR
