...
do_install:append:class-native () {
    ...
    # These are provided by llvm20-tblgen-native
    rm ${D}${bindir}/*-tblgen
}
...
PROVIDES:append:class-native = " llvm20-native"
PROVIDES:append:class-target = " llvm20"
PROVIDES:append:class-nativesdk = " nativesdk-llvm20"

PACKAGES =+ "${PN}-libllvm ${PN}-libclang-python ${PN}-libclang-cpp ${PN}-tidy ${PN}-format ${PN}-tools \
             libclang20 llvm20-linker-tools"
...
FILES:llvm20-linker-tools = "${libdir}/LLVMgold* ${libdir}/libLTO.so.* ${libdir}/LLVMPolly*"
