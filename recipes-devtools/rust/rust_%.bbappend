...
do_install:append:class-target() {
	...
	install -m 0755 ${WORKDIR}/rust-setup.py ${D}${datadir}/post-relocate-setup.d/

	install -d ${D}${libdir}/rustlib/${RUST_HOST_SYS}
	install -m 0644 ${WORKDIR}/target-sde.json \
...
do_install:append:class-nativesdk() {
	...
	install -m 0755 ${WORKDIR}/rust-setup.py ${D}${SDKPATHNATIVE}/post-relocate-setup.d/

	install -d ${D}${libdir}/rustlib/${SDK_PREFIX}gnu
	install -m 0644 ${WORKDIR}/target-sdk.json \
