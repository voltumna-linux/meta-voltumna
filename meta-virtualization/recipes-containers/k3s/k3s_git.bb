...
SRC_URI = "git://github.com/rancher/k3s.git;branch=release-1.36;name=k3s;protocol=https;destsuffix=${GO_SRCURI_DESTSUFFIX} \
           file://k3s.service \
...
SRCREV_k3s = "a2fc9574e060a325ac9998843efed7cd4dcb1c18"
...
# License-scan filter targets — do_compile builds exactly the one binary
# from ./cmd/server/main.go, which transitively imports the other ./cmd/*
# subcommands. `go list -deps` on this entry point gives the full set of
# modules whose code lands in the binary.
GO_MOD_DISCOVERY_LICENSE_TARGETS = "./cmd/server/main.go"
...
# Use `include` (not `require`) so the recipe parses on first-time enablement
# before discover_and_generate has been run to create the file.
