...
DEPENDS:append:class-target = " clang14-cross-${TARGET_ARCH} gcc-cross-${TARGET_ARCH} python3 compiler-rt14 libcxx14"
...
PACKAGES =+ "${PN}-libllvm ${PN}-lldb-python ${PN}-libclang-cpp ${PN}-tidy ${PN}-format ${PN}-tools \
             libclang14 lldb14 lldb14-server liblldb14 llvm14-linker-tools"

PROVIDES += "llvm14 llvm14-${PV}"
PROVIDES:append:class-native = " llvm14-native"
...
RDEPENDS:lldb14 += "${PN}-lldb-python lldb14-server"

RRECOMMENDS:${PN}-tidy += "${PN}-tools"

FILES:llvm14-linker-tools = "${libdir}/LLVMgold* ${libdir}/libLTO.so.* ${libdir}/LLVMPolly*"
...
INSANE_SKIP:${MLPREFIX}liblldb14 = "dev-so buildpaths"
...
TOOLCHAIN:class-nativesdk = "clang14"
